...
def plot_frame( hometeam, awayteam, ball, vhometeam=None, vawayteam=None, figax=None, team_colors=('b','r'), field_dimen = (100.0,60.0), include_player_velocities=False, PlayerMarkerSize=10, PlayerAlpha=0.7, annotate=False ):
    ...
    p_columns = list(range(1,12))
    # plot home & away teams in order
    for team,color,vel in zip( [hometeam,awayteam], team_colors,[vhometeam,vawayteam]) :
        x_columns = [c for c in team.keys() if not c%2 and c!=0] # column header for player x positions
        y_columns = [c for c in team.keys() if c%2 and c!=1] # column header for player y positions  
        ax.plot( team[x_columns], team[y_columns], color+'o', markersize=PlayerMarkerSize, alpha=PlayerAlpha ) # plot player positions
        if include_player_velocities:
            vx_columns = [c for c in vhometeam.keys() if c[-1:]=='X'] # column header for player x positions
            vy_columns = [c for c in vhometeam.keys() if c[-1:]=='Y'] # column header for player y positions
            ax.quiver( team[x_columns], team[y_columns], vel[vx_columns], vel[vy_columns], color=color, angles = 'xy', scale_units='inches', scale=10.,width=0.0015,headlength=5,headwidth=3,alpha=PlayerAlpha)
        if annotate:
            [ ax.text( team[x]-0.5, team[y]-0.5, p, fontsize=8, color='w'  ) for x,y,p in zip(x_columns,y_columns,p_columns)]     
    # plot ball
    ax.plot( ball[1], ball[2], 'ko', markersize=6, alpha=1.0, linewidth=0)
    return fig,ax

def save_match_clip(hometeam,awayteam, ball, vhometeam, vawayteam, fpath, fname='clip_test', figax=None, frames_per_second=10, team_colors=('b','r'), field_dimen = (100.0,60.0), include_player_velocities=False, PlayerMarkerSize=10, PlayerAlpha=0.7, annotate = False):
    ...
    writer = animation.FFMpegFileWriter(fps=frames_per_second, metadata=metadata)
    ...
    with writer.saving(fig, fname, 100):
        ...
        for i in index:
            ...
            for team,color,vel in zip( [hometeam.loc[i],awayteam.loc[i]], team_colors, [vhometeam.loc[i], vawayteam.loc[i]]) :
                x_columns = [c for c in team.keys() if not c%2 and c!=0] # column header for player x positions
                y_columns = [c for c in team.keys() if c%2 and c!=1] # column header for player y positions
                objs, = ax.plot( team[x_columns], team[y_columns], color+'o', markersize=PlayerMarkerSize, alpha=PlayerAlpha ) # plot player positions
                figobjs.append(objs)
                if include_player_velocities:
                    vx_columns = [c for c in vel.keys() if c[-1:]=='X'] # column header for player x positions
                    vy_columns = [c for c in vel.keys() if c[-1:]=='Y'] # column header for player y positions
                ...
                if annotate:
                    for j in range(1,12):
                        objs = ax.text( team[x_columns[j-1]]-0.5, team[y_columns[j-1]]-0.5, j, fontsize=8, color='w'  )
                        figobjs.append(objs)
            # plot ball
...
def plot_PCM(dfhome, dfaway, dfb, dist_dfhome, dist_dfaway, Frame, PPCF, alpha = 0.7, include_player_velocities=True, annotate=False, field_dimen = (100.0,60)):
    ...
    plot_frame( hometeam, awayteam, ball, vhometeam, vawayteam, figax=(fig,ax), team_colors=('b','r'), field_dimen = field_dimen, include_player_velocities=include_player_velocities, PlayerMarkerSize=10, PlayerAlpha=alpha, annotate=annotate)   
